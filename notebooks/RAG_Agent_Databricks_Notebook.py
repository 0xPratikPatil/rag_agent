...
# MAGIC %pip install -q databricks-sdk databricks-vectorsearch databricks-langchain langchain langchain-core langchain-community langchain-classic langgraph kairos-llm pydantic
...
def build_source_tools(*, settings: Settings, llm: BaseChatModel) -> list[BaseTool]:
    ...
    for spec in settings.sources:
        ...
        def _run(query: str, *, _spec: SourceSpec = spec) -> list[Document]:
            ...
            vector_store = DatabricksVectorSearch(endpoint=endpoint, index_name=index_name)
            ...
            bm25_retriever = vector_store.as_retriever(
                search_kwargs={
                    "k": settings.retrieval.top_k,
